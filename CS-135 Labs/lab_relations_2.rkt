...
;; Check the bottom of this file for helper functions you can use.

;; NOTE: If you can't get a certain function to work before the deadline,
;; DO NOT DELETE IT! Just put a string in the body of the function.
;;***************************************************************************
...
(define (composite relationOuter relationInner)
  (if (null? relationOuter)
      '()
      (append (compositeHelp (car relationOuter) relationInner) (composite (cdr relationOuter) relationInner))))

(define (compositeHelp tuple relationInner)
  (if (null? relationInner)
      '()
      (if (equal? (cadar relationInner) (car tuple))
          (cons (list (caar relationInner) (cadr tuple)) (compositeHelp tuple (cdr relationInner)))
          (compositeHelp tuple (cdr relationInner)))))
...
(define (transitive-closure relation)
  (transitive-closureHelp relation (length relation)))

(define (transitive-closureHelp relation n)
  (if (equal? n 0)
      '()
   (union (power relation n) (transitive-closureHelp relation (- n 1)))))
...
(define (transitive? relation)
   (equal? relation (transitive-closure relation)))
...
(define (EQ-relation? relation n)
  (and (transitive? relation) (reflexive? relation n)))


;;_____________________________________________________________________

;; Helper functions you may utilize for the functions you write!
