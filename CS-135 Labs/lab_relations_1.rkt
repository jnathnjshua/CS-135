...
;; This can be done with simple recursion and using the (reverse list) function
...
(define (symmetric-closure relation)
  (make-set (union(flip-pairs relation) relation)))

;; Define related-to
;; related-to takes an element and relation and returns the list of elements that the given element is related to.
;; For an element x, every y is related to x if pair (x y) is in the relation.
...
(define (related-to element relation)
   ...
   (if (equal? (car (car relation)) element)
       (cons (car (reverse (car relation))) (related-to element (cdr relation)))
